...
public class EnemyScript : MonoBehaviour
{
    ...
    private void AttackFunc()
    {
        ...
        if (attack_time <= 0f)
        {
            ...
            if (isRanged)
            {
                ...
                if (transform.position.x > player_Script.transform.position.x)
                    fire.GetComponent<MissiblesRanges>().setVariables(Mathf.Abs(speedMissible) * (-1f), distanceMissible, damage,missible_sfx);
                else
                    fire.GetComponent<MissiblesRanges>().setVariables(Mathf.Abs(speedMissible), distanceMissible, damage,missible_sfx);
            ...
            {
                bool damageTake = Physics2D.OverlapCircle(transform.GetChild(0).position, 2f, 1 << LayerMask.NameToLayer("Player"));
                if (damageTake)
                {
    ...
    void Update()
    {
        ...
        //          death            //
        //===========================//
        if (!PlayerScript.allPause && player_Script.gameObject != null)
        {
            if (isDeath)
            {
                if (death_time <= 0)
            ...
            {
                if (!PlayerScript.isDeath && player_Script != null)
                {
                    //===========================//
                    //          attack           //
                    //===========================//
